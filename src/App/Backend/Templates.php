...
class Templates extends Base {
	...
	public function init() {
		/**
		 * This backend class is only being instantiated in the frontend as requested in the Bootstrap class
		 *
	...
	public function get( $slug, $name = null, $args = [], $load = true ): string {
		...
		do_action( 'prisjakt_feed_get_template_part_' . $slug, $slug, $name, $args );
		// Get files names of templates, for given slug and name.
		$templates = $this->get_file_names( $slug, $name, $args );
	 ...
	 * @since 1.0.0
	 */
	protected function get_file_names( $slug, $name, $args ): array {
		$templates = [];
		if ( isset( $name ) ) {
		 ...
		 * @since 1.0.0
		 */
		return apply_filters( 'prisjakt_feed_get_template_part', $templates, $slug, $name, $args );
	}
	...
	protected function get_paths(): array {
		$theme_directory = trailingslashit( $this->plugin->ext_template_folder() );

		$file_paths = [
			10  => trailingslashit( get_template_directory() ) . $theme_directory,
			100 => $this->plugin->template_path(),
		];
		 ...
		 * @since 1.0.0
		 */
		$file_paths = apply_filters( 'prisjakt_feed_template_paths', $file_paths );
